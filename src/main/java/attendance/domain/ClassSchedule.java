...
public enum ClassSchedule {
    ...
    ClassSchedule(final List<Integer> dayOfWeekValues, final LocalTime startTime, final LocalTime endTime) {
    ...
    public static int calculateTimeBetween(LocalDateTime dateTime) {
        ClassSchedule schedule = findTypeByDayOfWeek(dateTime.getDayOfWeek());
        validationAfterClass(dateTime.toLocalTime(), schedule);

        Duration between = Duration.between(schedule.startTime, dateTime.toLocalTime());
        return (int) between.toMinutes();
    }

    private static void validationAfterClass(final LocalTime time, final ClassSchedule schedule) {
        if (time.isAfter(schedule.endTime)) {
            throw new IllegalArgumentException(AFTER_CLASS_TIME.get());
        }
    }

    private static ClassSchedule findTypeByDayOfWeek(final DayOfWeek dayOfWeek) {
