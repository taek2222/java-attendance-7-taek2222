...
- [x] 날짜(일) 동일한 크루 출석 수정 
- [x] 변경 전과 변경 후의 기록 출력 `12월 03일 화요일 10:07 (지각) -> 09:58 (출석) 수정 완료!`
- [x] `예외처리` 수정일이 주말 또는 공휴일인 경우 예외가 발생한다 `12월 14일 토요일은 등교일이 아닙니다.`
- [x] `예외처리` 잘못된 날짜(일) 입력할 경우 예외가 발생한다 `잘못된 형식을 입력하였습니다.`
- [x] `예외처리` 미래 날짜를 수정한 경우 예외가 발생한다 `아직 수정할 수 없습니다.`
```
...
### ✅ 제적 위험자 확인 기능
- [x] 지각 및 결석 횟수 카운팅
- [x] 횟수에 따라 경고, 면담, 제적 계산 (지각 3회 == 결석 1회)
    - 경고 대상자: 결석 2회 이상
    - 면담 대상자: 결석 3회 이상
    - 제적 대상자: 결석 5회 초과
- [x] 전날까지의 크루 출석 기록을 바탕으로 제적 위험자 파악
- [x] 제적 위험자 -> 면담 대상자 -> 경고 대상자 순서로 출력 
